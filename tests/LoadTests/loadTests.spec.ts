...
test("after I load a correct file into the input box, the correct output is printed", async ({
  ...
  // load good file
  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill("load_file headerCSV.csv");
...
test("after I load an incorrect file into the input box, the error output is printed", async ({page }) => {
  //go to page
  await page.goto("http://localhost:8000/");

  // load a bad csv
  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill("load_file selrjkrdfj.csv");
...
// test bad load
test("after I load an empty path into the input box, the error output is printed", async ({ page }) => {
  // go to page
  await page.goto("http://localhost:8000/");
...
test("load one valid CSV then a different valid CSV should print two correct outputs", async ({ page }) => {
  // go to page
  await page.goto("http://localhost:8000/");
  ...
  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill("load_file doubleCSV.csv");
...
test("loading one valid CSV then an invalid CSV should print a success then a error", async ({ page }) => {
  // go to page
  await page.goto("http://localhost:8000/");
  ...
  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill("load_file asdfadsf.csv");
...
test("loading one invalid CSV then a valid CSV should print an error then a success", async ({ page }) => {
  // go to apge
  await page.goto("http://localhost:8000/");

  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill("load_file sdfadsfa.csv");
