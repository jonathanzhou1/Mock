...
test("load mode view mode", async ({ page }) => {
  ...
  await expect(body1.nth(0).nth(0)).toHaveText("Inputted Command: load_file headerCSV.csv");
  ...
  await expect(body2.nth(0).nth(0)).toHaveText("Inputted Command: view");
  ...
  const body3 = await page.locator(".output3").locator("table").locator("tbody").locator("tr").locator("td")
...
test("Search multiple times on the same load", async ({ page }) => {
  ...
  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill("search {isaac yi}");
  await button.click();
  ...
  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill("search {18}");
  await button.click();
...
//NEXT PROJECT: Test CSV and ACS functionality together
